...
<div class="card" style="max-width: 600px;">
    <form method="POST" action="{{ route('goals.update', $goal) }}">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem;">
            <div class="form-group">
                ...
                <select id="month" name="month" class="form-select" required>
                    ...
                    @for($i = 1; $i <= 12; $i++)
                        <option value="{{ $i }}" @selected(old('month', $goal->month) == $i)>{{ ['Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro'][$i-1] }}</option>
                    @endfor
                </select>
            ...
            <div class="form-group">
                ...
                <input type="number" id="year" name="year" class="form-input" value="{{ old('year', $goal->year) }}" required>
        ...
        <div class="form-group">
            ...
            <input type="number" id="target" name="target" class="form-input" value="{{ old('target', $goal->target) }}" min="1" required>
            @error('target') <span style="color: #ef4444; font-size: 0.875rem;">{{ $message }}</span> @enderror
        </div>

        <div style="display: flex; gap: 1rem; margin-top: 2rem;">
            <button type="submit" class="btn-primary">Atualizar Meta</button>
